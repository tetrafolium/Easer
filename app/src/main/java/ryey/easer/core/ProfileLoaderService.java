...
public class ProfileLoaderService extends Service {
  public static final String ACTION_LOAD_PROFILE =
      "ryey.easer.action.LOAD_PROFILE";

  public static final String EXTRA_PROFILE_NAME =
      "ryey.easer.extra.PROFILE_NAME";
      ...
      if (ACTION_LOAD_PROFILE.equals(action)) {
        ...
        if (intent.getExtras() == null) {
          ...
          throw new IllegalStateException(
              "ProfileLoaderIntent has null extras???");
        }
        handleActionLoadProfile(name, event, intent.getExtras());
      } else {
        Logger.wtf("ProfileLoaderService got unknown Intent action <%s>",
                   action);
      }
    }
  };
  ...
  public class PLSBinder extends Binder {
    ...
    public void triggerProfile(final String profileName,
      ...
      if (dynamicsProperties.containsKey(
              EXTRA_DYNAMICS_PROPERTIES)) // TODO: Make the semantics better so
  ...
  private void handleActionLoadProfile(final @NonNull String name,
                                       final @Nullable String event,
    ...
    final SolidDynamicsAssignment solidMacroAssignment =
        dynamicsLink.assign(macroData);

    ProfileLoadWatcher profileLoadWatcher =
        new ProfileLoadWatcher(this, name, event);

    new Thread(profileLoadWatcher).start();

    for (String pluginId : profile.pluginIds()) {
      int count =
          helper.triggerOperations(pluginId, profile.get(pluginId),
                                   solidMacroAssignment, profileLoadWatcher);
      profileLoadWatcher.addTaskCount(count);
    }
    profileLoadWatcher.finishInit();
  }

  class ProfileLoadWatcher
      implements Runnable,
                 SkillHelper.OperationHelper.OnOperationLoadingResultListener {
    ...
    ProfileLoadWatcher(final Context context, final String name,
                       final String event) {
    ...
    void addTaskCount(final int count) { this.taskCount.addAndGet(count); }

    void finishInit() { lock.unlock(); }
    ...
    private void allResultsCollected() {
      ...
      if (event == null) {
        ActivityLogService.Companion.notifyProfileLoaded(context, name,
                                                         extraInfo);
      } else {
        ActivityLogService.Companion.notifyScriptSatisfied(context, event, name,
                                                           extraInfo);
      }
    }

...
public class RemotePluginCommunicationHelper {
  ...
  public static class C {
    ...
    public static final String EXTRA_PLUGIN_ID =
        "ryey.easer.IPC.EXTRA.PLUGIN_ID";
    public static final String EXTRA_PLUGIN_INFO =
        "ryey.easer.IPC.EXTRA.PLUGIN_INFO";
    ...
    public static final String EXTRA_PLUGIN_LIST =
        "ryey.easer.IPC.EXTRA.PLUGIN_LIST";
    ...
    public static final String EXTRA_MESSAGE_ID =
        "ryey.easer.IPC.EXTRA.MESSAGE_ID";
    public static final String EXTRA_RAW_DATA = "ryey.easer.IPC.EXTRA.RAW_DATA";
    public static final String EXTRA_PLUGIN_DATA =
        "ryey.easer.IPC.EXTRA.PLUGIN_DATA";
    ...
    public static final String EXTRA_PLUGIN_PACKAGE =
        "ryey.easer.IPC.EXTRA.PLUGIN_PACKAGE";
    public static final String EXTRA_PLUGIN_EDIT_DATA_ACTIVITY =
        "ryey.easer.IPC.EXTRA.PLUGIN_EDIT_DATA_ACTIVITY";
  }
  ...
  private ServiceConnection serviceConnection = new ServiceConnection() {
    ...
    public void onServiceConnected(final ComponentName componentName,
      ...
      Logger.d("[RemotePluginCommunicationHelper:%s] onServiceConnected",
               context);
  ...
  private DelayedTaskUntilConnectedWrapper delayedTaskUntilConnectedWrapper =
      new DelayedTaskUntilConnectedWrapper();
  private void doAfterConnect(final Callable<Void> task) {
    delayedTaskUntilConnectedWrapper.doAfterConnected(task);
  }

  private final DelayedCallback<OnPluginFoundCallback>
      onPluginFoundCallbackDelayedCallback =
          new DelayedCallback<>(new ArrayMap<>());
  private final DelayedCallback<OnOperationPluginListObtainedCallback>
      onOperationPluginListObtainedCallbackDelayedCallback =
          new DelayedCallback<>(new ArrayMap<>());
  private final DelayedCallback<OnEditDataIntentObtainedCallback>
      onEditDataIntentObtainedCallbackDelayedCallback =
          new DelayedCallback<>(new ArrayMap<>());
  private final DelayedCallback<OnOperationDataParsedCallback>
      onOperationDataParsedCallbackDelayedCallback =
          new DelayedCallback<>(new ArrayMap<>());
  ...
  public void onConnectedParseOperationData(
      final String id, final String rawData,
      final OnOperationDataParsedCallback onOperationDataParsedCallback) {
    ParcelUuid uuid = onOperationDataParsedCallbackDelayedCallback.putCallback(
        onOperationDataParsedCallback);
  ...
  asyncFindPlugin(final String id,
                  final OnPluginFoundCallback onPluginFoundCallback) {
    ParcelUuid uuid =
        onPluginFoundCallbackDelayedCallback.putCallback(onPluginFoundCallback);
    doAfterConnect(new Callable<Void>() {
      @Override
      public Void call() throws Exception {
        Message message = Message.obtain();
  ...
  synchronized public void
  asyncCurrentOperationPluginList(final
                                  @NonNull OnOperationPluginListObtainedCallback
                                      onOperationPluginListObtainedCallback) {
    ParcelUuid uuid =
        onOperationPluginListObtainedCallbackDelayedCallback.putCallback(
            onOperationPluginListObtainedCallback);
    doAfterConnect(new Callable<Void>() {
      @Override
      public Void call() throws Exception {
  ...
  public void asyncTriggerOperation(final String id,
                                    final RemoteOperationData data) {
    doAfterConnect(new Callable<Void>() {
      @Override
      public Void call() throws Exception {
        Message message = Message.obtain();
  ...
  public void asyncRemoteEditOperationData(
      final String id,
      final OnEditDataIntentObtainedCallback onEditDataIntentObtainedCallback) {
    ParcelUuid uuid =
        onEditDataIntentObtainedCallbackDelayedCallback.putCallback(
  ...
  public void end() { context.unbindService(serviceConnection); }

  static class IncomingHandler extends Handler {
    WeakReference<RemotePluginCommunicationHelper> ref;

    IncomingHandler(
        final WeakReference<RemotePluginCommunicationHelper> reference) {
      ref = reference;
    }
    ...
    public void handleMessage(final Message msg) {
      ...
      if (msg.what == C.MSG_FIND_PLUGIN_RESPONSE) {
        ...
        RemotePluginInfo info =
            msg.getData().getParcelable(C.EXTRA_PLUGIN_INFO);
        ParcelUuid uuid = msg.getData().getParcelable(C.EXTRA_MESSAGE_ID);
        assert uuid != null;
        OnPluginFoundCallback callback =
            ref.get().onPluginFoundCallbackDelayedCallback.retrieveCallBack(
                uuid);
        if (callback != null)
          callback.onPluginFound(info);
      } else if (msg.what == C.MSG_CURRENT_OPERATION_PLUGIN_LIST_RESPONSE) {
        msg.getData().setClassLoader(
            RemoteOperationPluginInfo
                .class.getClassLoader()); // Required (for strange reason);
                                          // otherwise ClassNotFound
        ArrayList<RemoteOperationPluginInfo> infoList =
            msg.getData().getParcelableArrayList(C.EXTRA_PLUGIN_LIST);
        Set<RemoteOperationPluginInfo> infoSet = new ArraySet<>(infoList);
        ParcelUuid uuid = msg.getData().getParcelable(C.EXTRA_MESSAGE_ID);
        assert uuid != null;
        OnOperationPluginListObtainedCallback callback =
            ref.get()
                .onOperationPluginListObtainedCallbackDelayedCallback
                .retrieveCallBack(uuid);
        if (callback != null)
          callback.onListObtained(infoSet);
      } else if (msg.what == C.MSG_PARSE_OPERATION_DATA_RESPONSE) {
        OperationData operationData =
            msg.getData().getParcelable(C.EXTRA_PLUGIN_DATA);
        assert operationData != null;
        ParcelUuid uuid = msg.getData().getParcelable(C.EXTRA_MESSAGE_ID);
        OnOperationDataParsedCallback callback =
            ref.get()
                .onOperationDataParsedCallbackDelayedCallback.retrieveCallBack(
                    uuid);
        if (callback != null)
  ...
  static class DelayedTaskUntilConnectedWrapper
      extends AsyncHelper.DelayedWhenSatisfied {
    void onConnected(final Messenger outMessenger) { super.onSatisfied(); }

    void onDisconnected() { super.onUnsatisfied(); }

    void doAfterConnected(final Callable<Void> task) { super.doAfter(task); }
  }

 ...
 * It maintains several {@link Lotus}(es) and send Intent to load Profile(s) &
 * etc.
 */
public class EHService extends Service {
  ...
  public static final String ACTION_PROFILE_UPDATED =
      "ryey.easer.action.PROFILE_UPDATED";
  ...
  private final DelayedConditionHolderBinderJobs jobCH =
      new DelayedConditionHolderBinderJobs();
  private final AsyncHelper.DelayedLoadProfileJobs jobLP =
      new AsyncHelper.DelayedLoadProfileJobs();
  private final ServiceConnection connection = new ServiceConnection() {
    ...
    public void onServiceConnected(final ComponentName componentName,
      ...
      if (componentName.getClassName().equals(
              ConditionHolderService.class.getName())) {
        jobCH.onBind((ConditionHolderService.CHBinder)iBinder);
        jobCH.doAfter(() -> {
          mSetTriggers();
          return null;
        });
      } else if (componentName.getClassName().equals(
                     ProfileLoaderService.class.getName())) {
  ...
  public static void
  registerConditionEventNotifier(final @NonNull Context context,
                                 final @NonNull String conditionName,
                                 final @NonNull Uri notifyData) {
    CoreSkillHelper.registerConditionEventNotifier(context, conditionName,
                                                   notifyData);
  }
  public static void
  unregisterConditionEventNotifier(final @NonNull Context context,
                                   final @NonNull String conditionName,
                                   final @NonNull Uri notifyData) {
    CoreSkillHelper.unregisterConditionEventNotifier(context, conditionName,
                                                     notifyData);
  }
  ...
  public void onCreate() {
    ...
    ActivityLogService.Companion.notifyServiceStatus(this, SERVICE_NAME, true,
                                                     null);
    bindService(new Intent(this, ConditionHolderService.class), connection,
                Context.BIND_AUTO_CREATE);
    bindService(new Intent(this, ProfileLoaderService.class), connection,
                Context.BIND_AUTO_CREATE);
    running = true;
  ...
  public void onDestroy() {
    ...
    ActivityLogService.Companion.notifyServiceStatus(this, SERVICE_NAME, false,
                                                     null);
    mCancelTriggers();
  ...
  private void mSetTriggers() {
    ...
    for (ScriptTree script :
         scriptTreeList) { // TODO?: Move this to `FakeRootLotus`
      Logger.v(TAG + "setting trigger for <%s>", script.getName());
      if (script.isActive()) {
        Lotus lotus =
            Lotus.createLotus(this, script, executorService, jobCH, jobLP);
        lotus.listen();
  ...
  public class EHBinder extends Binder {
     ...
     */
    public boolean setLotusStatus(final String eventName,
    ...
    private boolean setLotusStatus_real(final Lotus lotus,
  ...
  static class DelayedConditionHolderBinderJobs
      extends AsyncHelper
                  .DelayedServiceBinderJobs<ConditionHolderService.CHBinder> {}

  private static class CoreSkillHelper {
    private static final String ACTION_UNREGISTER_CONDITION_EVENT =
        "ryey.easer.service.action.UNREGISTER_CONDITION_EVENT";
    private static final String ACTION_REGISTER_CONDITION_EVENT =
        "ryey.easer.service.action.REGISTER_CONDITION_EVENT";
    private static final String EXTRA_CONDITION_NAME =
        "ryey.easer.service.extra.CONDITION_NAME";
    private static final String EXTRA_NOTIFY_DATA =
        "ryey.easer.service.extra.NOTIFY_DATA";
    ...
    /**
     * For {@link
     * ryey.easer.skills.event.condition_event.ConditionEventEventSkill}
     */
    final BroadcastReceiver mReceiver = new BroadcastReceiver() {
      @Override
      public void onReceive(final Context context, final Intent intent) {
        ...
        if (ACTION_REGISTER_CONDITION_EVENT.equals(intent.getAction()) ||
            ACTION_UNREGISTER_CONDITION_EVENT.equals(intent.getAction())) {
          ...
          if (ACTION_REGISTER_CONDITION_EVENT.equals(intent.getAction()))
            service.jobCH.doAfter(
                binder
                -> binder.registerAssociation(conditionName, notifyData));
          else
            service.jobCH.doAfter(
                binder
                -> binder.unregisterAssociation(conditionName, notifyData));
        }
      }
    };
    ...
    private CoreSkillHelper(final EHService service) { this.service = service; }

    /**
     * For {@link ryey.easer.skills.event.widget.WidgetEventSkill}
     */
    WidgetBroadcastRedispatcher widgetBroadcastRedispatcher =
        new WidgetBroadcastRedispatcher();

    void onCreate() {
      widgetBroadcastRedispatcher.start(service);
      LocalBroadcastManager.getInstance(service).registerReceiver(
          mReceiver, filter_conditionEvent);
    }
    ...
    private static class WidgetBroadcastRedispatcher {
      /**
       * Handles the broadcast (from PendingIntent) from any widgets, by
       * redispatching This is required because AppWidget can only send
       * PendingIntent to a specific Component (EHService)
       */
      ...
      void start(final Context context) {
        ...
        intentFilter.addAction(
            UserActionWidget.Companion.getACTION_WIDGET_CLICKED());

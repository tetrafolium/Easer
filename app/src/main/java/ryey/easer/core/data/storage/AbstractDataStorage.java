...
public abstract class AbstractDataStorage<
    T extends Named & Verifiable & WithCreatedVersion, T_backend
        extends DataStorageBackendCommonInterface<T>> {

  protected final Context context;
  protected final T_backend[] storage_backend_list;

  protected AbstractDataStorage(final Context context,
                                final T_backend[] storage_backend_list) {
  ...
  public T get(final String name) {
    for (T_backend backend : storage_backend_list) {
      try {
        ...
        Logger.v("data not found on backend <%s>",
   ...
   * @return {@code true} if {@param name} is safely deleted, {@code false} if
   *     it is used by other events.
   */
   ...
   * @param data The {@link T} of the new data (whose name may be different with
   *     {@param oldName} because of user's change).
  ...
  /**
   * Update an existing {@link T} with the new {@param data} without changing
   * the name.
  ...
  protected abstract void handleRename(String oldName, T data)
      throws IOException;
}

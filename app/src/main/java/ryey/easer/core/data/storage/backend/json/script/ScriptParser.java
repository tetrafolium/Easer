...
class ScriptParser implements Parser<ScriptStructure> {
  ...
  ScriptParser(final Context context) { this.context = context; }

  public ScriptStructure parse(final InputStream in)
    ...
    try {
      ...
      if (version < C.VERSION_USE_SCENARIO) { // Can be removed (because this is
                                              // covered by the else statement)
        EventData eventData =
            parse_eventData(jsonObject.getJSONObject(C.TRIG), version);
  ...
  private void parseAndSet_trigger(final JSONObject jsonObject_trigger,
                                   final int version)
    ...
    try {
      ...
      case C.TriggerType.T_CONDITION:
        ...
        ConditionStructure condition =
            new ConditionDataStorage(context).get(condition_name);
  ...
  EventData parse_eventData(final JSONObject jsonObject_trigger,
                            final int version)
      throws IllegalStorageDataException {
    try {
      ...
      return LocalSkillRegistry.getInstance()
          .event()
          .findSkill(spec)
          .dataFactory()
          .parse(jsonObject_situation.getString(C.DATA), PluginDataFormat.JSON,
                 version);
    } catch (JSONException e) {
      throw new IllegalStorageDataException(e);
    }

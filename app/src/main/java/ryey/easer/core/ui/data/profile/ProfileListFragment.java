...
static { TAG = "[ProfileListFragment] "; }

AsyncHelper.DelayedLoadProfileJobsWrapper loadProfileJobWrapper =
	new AsyncHelper.DelayedLoadProfileJobsWrapper();
...
protected List<ListDataWrapper> queryDataList() {
	...
	for (String name : dataStorage.list()) {
		...
		if (valid) {
			Collection<RemoteLocalOperationDataWrapper> stateControlOperationData =
				profile.get(new StateControlOperationSkill().id());
			if (stateControlOperationData.size() > 0) {
				for (RemoteLocalOperationDataWrapper dataWrapper :
				     stateControlOperationData) {
					StateControlOperationData operationData =

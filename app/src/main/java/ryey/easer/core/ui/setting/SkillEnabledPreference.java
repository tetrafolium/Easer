...
class SkillEnabledPreference extends CheckBoxPreference
  ...
  SkillEnabledPreference(final Context context, final Skill skill,
                         final boolean in_use) {
    super(context);
    this.skill = skill;
    this.in_use = in_use;
  ...
  protected void onBindView(final View view) {
    ...
    if (skill instanceof OperationSkill) {
      ...
      if (privilege == PrivilegeUsage.root_only ||
          privilege == PrivilegeUsage.prefer_root) {
  ...
  boolean redrawPermissionButton() {
    ...
    if (ret == null) {
      ...
      if (ret) {
        btnPermission.setImageDrawable(getContext().getResources().getDrawable(
            R.drawable.ic_status_positive_inner));
      } else {
        btnPermission.setImageDrawable(getContext().getResources().getDrawable(
            R.drawable.ic_status_negative_inner));
      }
    }
    return true;
  }

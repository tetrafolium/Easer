	...
	if (ContextCompat.checkSelfPermission(context, permission) !=
		...
		.makeText(context,
		          String.format(context.getString(
						R.string.prompt_prevented_for_permission),
		                        permission),
		          Toast.LENGTH_LONG)
		.show();
		return false;
...
onSharedPreferenceChanged(final SharedPreferences sharedPreferences,
	...
	if (key.equals(getString(R.string.key_pref_autostart))) {
		ComponentName componentName =
			new ComponentName(this, BootUpReceiver.class);
		...
		ComponentName componentName =
			new ComponentName(this, UpgradeCompleteReceiver.class);
		PackageManager pm = getPackageManager();
		if (sharedPreferences.getBoolean(key, false)) {
			pm.setComponentEnabledSetting(
				componentName, PackageManager.COMPONENT_ENABLED_STATE_ENABLED,
...
static void setSkillFragment(final FragmentManager fragmentManager,
                             final boolean addToBackStack) {
	SkillSettingsPreferenceFragment fragment =
		new SkillSettingsPreferenceFragment();
...
public void onCreate(final @Nullable Bundle savedInstanceState) {
	...
	try {
		...
		Preference pref_version =
			findPreference(getString(R.string.key_pref_version));
	...
	Preference pref_export =
		findPreference(getString(R.string.key_pref_export));
	...
	Preference pref_import =
		findPreference(getString(R.string.key_pref_import));
	pref_import.setOnPreferenceClickListener(
		new Preference.OnPreferenceClickListener() {
				@Override
				public boolean onPreferenceClick(final Preference preference) {
				        if (!hasPermission(getActivity(),
	...
	Preference pref_logging =
		findPreference(getString(R.string.key_pref_logging));
	pref_logging.setOnPreferenceChangeListener(
				        ...
				        try {
							...
							throw new NumberFormatException();
				                ...
				                .makeText(getActivity(),
				                          R.string.cooldown_time_illformed,
				                          Toast.LENGTH_SHORT)
				                .show();
				                return false;
					}
		...
		new Preference.OnPreferenceClickListener() {
				...
				public boolean onPreferenceClick(final Preference preference) {
				        if (!StorageHelper.convertToNewData(getActivity())) {
				                Toast
				                .makeText(getActivity(),
				                          R.string.message_convert_data_error,
				                          Toast.LENGTH_LONG)
				                .show();
					}
				        return true;
	...
	.setOnPreferenceChangeListener(
		new Preference.OnPreferenceChangeListener() {
				...
				public boolean onPreferenceChange(final Preference preference,
				        ...
				        Logger.d("Locale changing to %s, based on %s", locale,
				                 locale_str);
				        Context context;
...
private static void export_data(final Context context) {
	try {
		...
		Toast
		.makeText(context,
		          String.format(context.getString(R.string.template_export),
		                        dest.getName()),
		          Toast.LENGTH_SHORT)
		.show();
	} catch (IOException e) {

...
public class VolumeOperationData implements OperationData {
  ...
  VolumeOperationData(final Integer vol_ring, final Integer vol_media,
                      final Integer vol_alarm, final Integer vol_notification,
                      final Integer vol_bt, final Integer bt_delay) {
  ...
  VolumeOperationData(final @NonNull String data,
                      final @NonNull PluginDataFormat format, final int version)
      throws IllegalStorageDataException {
    parse(data, format, version);
  }

  private static Integer optInteger(final JSONObject jsonObject,
                                    final String key) {
  ...
  private static void writeNonNull(final JSONObject jsonObject,
                                   final Integer value, final String key)
  ...
  public boolean equals(final Object obj) {
    ...
    if (!Utils.nullableEqual(vol_alarm, ((VolumeOperationData)obj).vol_alarm))
      return false;
    if (!Utils.nullableEqual(vol_notification,
                             ((VolumeOperationData)obj).vol_notification))
      return false;
  ...
  public boolean isValid() {
    return (isNotNegative(vol_ring) || isNotNegative(vol_media) ||
            isNotNegative(vol_alarm) || isNotNegative(vol_notification) ||
            isNotNegative(vol_bt) || isNotNegative(bt_delay)) &&

...
public class HeadsetTracker extends SkeletonTracker<HeadsetUSourceData> {
  ...
  private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
    ...
    public void onReceive(final Context context, final Intent intent) {
      ...
      if (expected_action.equals(intent.getAction()) && extras != null) {
        ...
        final Boolean match =
            determine_match(data, state == 1, microphone == 1);
  ...
  static Boolean determine_match(final HeadsetUSourceData data,
                                 final boolean plugging_in,
                                 final @Nullable Boolean has_microphone) {
    if (!(data.hs_state == HeadsetUSourceData.HeadsetState.any ||
          plugging_in ==
              (data.hs_state == HeadsetUSourceData.HeadsetState.plug_in)))
      return false;

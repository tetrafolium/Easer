...
enum HeadsetState {
	...
	plug_out,
	any,
	@Deprecated plugged_in,
	@Deprecated plugged_out,
}
...
@Deprecated private static final String K_HS_ACTION = "headset_action";
...
HeadsetUSourceData(final @NonNull HeadsetState hs_state,
                   final @NonNull HeadsetType hs_type) {
	this.hs_state = hs_state;
	this.hs_type = hs_type;
}

HeadsetUSourceData(final @NonNull String data,
...
@NonNull
private static HeadsetState getHSState(final JSONObject jsonObject,
                                       final int version)
throws JSONException {
	...
	if (version < C.VERSION_UNIFORMED_SOURCE) {
		...
		if (hs_state == HeadsetState.plugged_in ||
		    hs_state == HeadsetState.plugged_out) {
			Logger.e(
				"HeadsetUSource data (after USource) uses deprecated HeadsetState");
		}
	}
	return hs_state;
}

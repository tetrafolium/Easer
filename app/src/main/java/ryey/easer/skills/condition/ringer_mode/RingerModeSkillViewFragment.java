...
public class RingerModeSkillViewFragment
	extends SkillViewFragment<RingerModeConditionData> {

RadioGroup rg_ringer_mode, rg_volume_match_type;
RadioButton rb_ringer_mode_silent, rb_ringer_mode_vibrate,
            rb_ringer_mode_normal, rb_volume_match_above, rb_volume_match_below,
            rb_volume_match_equals;
SeekBar sb_volume_match_value;
TextView tv_volume_match_value;

private void setRingerOptionsState() {
	if (rg_ringer_mode.getCheckedRadioButtonId() ==
	    rb_ringer_mode_normal.getId()) {
...
public View onCreateView(final @NonNull LayoutInflater inflater,
	...
	View view = inflater.inflate(R.layout.plugin_condition__ringer_mode,
	                             container, false);
	...
	rg_ringer_mode.setOnCheckedChangeListener(
		(group, checkedId)->{ setRingerOptionsState(); });

	int maxVol =
		(am != null) ? am.getStreamMaxVolume(AudioManager.STREAM_RING) : 0;
	sb_volume_match_value.setMax(maxVol);
	sb_volume_match_value.setOnSeekBarChangeListener(
	...
	tv_volume_match_value.setText(
		String.valueOf(sb_volume_match_value.getProgress()));
	return view;
}

 ...
 * Slots also carry the duty to receive the relevant signals (e.g. Intent
 * broadcasts of Android) and notify the holder to proceed to check.
 *
 * Slots are used in {@link ryey.easer.core.Lotus}.
 */
public abstract class AbstractSlot<T extends EventData> implements Slot<T> {
  /**
   * AndroidStudio reminds me that some fields and/or methods can be made
   * private. I'm not sure if they will be used by the subclasses in the future
   * when extending the `satisfied` field to more status, so they are left as
   * protected.
   */
  ...
  /**
   * Controls whether the current slot could be re-triggered if it is already in
   * a satisfied state. If uncertain, make it re-triggerable
   */
   ...
   * Only the to-level (in the {@link ScriptTree}) slot will need this (to tell
   * the {@link ryey.easer.core.Lotus} to check the whole tree).
   */
  protected Uri notifyLotusData;

  public AbstractSlot(final @NonNull Context context,
                      final @ValidData @NonNull T data,
   ...
   * It will notify the corresponding {@link ryey.easer.core.Lotus} (by emitting
   * {@link android.content.Intent} with {@link #notifyLotusData} as data) iif
   * the satisfaction state is changed.
   *
   * This method sets the {@link #satisfied} variable.
   */
  protected synchronized void
  changeSatisfiedState(final boolean newSatisfiedState, final Bundle dynamics) {
    if (satisfied != null) {
      if (persistent && satisfied && !newSatisfiedState) {
        Logger.v(
            "prevent from resetting a persistent slot back to unsatisfied");
        return;
      }

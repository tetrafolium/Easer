...
public class CalendarSkillViewFragment
	extends SkillViewFragment<CalendarEventData> {

private final static String ACTION_RETURN =
	"ryey.easer.skills.combined.bluetooth_device.return_from_dialog";
private final static String EXTRA_CALENDAR_ID =
	"ryey.easer.skills.event.calendar.extra.calendar_id";
private final static String EXTRA_CALENDAR_NAME =
	"ryey.easer.skills.event.calendar.extra.calendar_name";

private long calendar_id = -1;
private TextView tv_calendar_name;
private final CheckBox[] cb_conditions =
	new CheckBox[CalendarData.condition_name
	             .length];   // The same order as
                                 // `CalendarData.condition_name`
...
public View onCreateView(final @NonNull LayoutInflater inflater,
	...
	View view =
		inflater.inflate(R.layout.plugin_event__calendar, container, false);
	...
	view.findViewById(R.id.calendar_picker)
	.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(final View v) {
			        if (!SkillUtils.checkPermission(getContext(),
					...
					getContext(), CalendarContract.Calendars.CONTENT_URI,
					new String[] {
					...
					0, new Loader.OnLoadCompleteListener<Cursor>() {
					@Override
					public void onLoadComplete(final Loader<Cursor> loader,
					                           final Cursor data) {
					        simpleCursorAdapter.swapCursor(data);
					}
				});
			        builderSingle.setAdapter(
					simpleCursorAdapter, new DialogInterface.OnClickListener() {
					@Override
...
protected void _fill(final @ValidData @NonNull CalendarEventData data) {
	...
	for (int i = 0; i < CalendarData.condition_name.length; i++) {
		cb_conditions[i].setChecked(
			calendarData.conditions.contains(CalendarData.condition_name[i]));
	}
}

...
public class TcpTripSlot extends AbstractSlot<TcpTripEventData> {
  ...
  private class PostSend extends Thread {
    ...
    PostSend(final Socket socket) { this.socket = socket; }
    ...
    public void run() {
      if (!eventData.check_reply) {
        ...
        do { // waiting for input
          Logger.v("closed? %s :: inputShutdown? %s", socket.isClosed(),
          ...
          try {
            if (reader.ready()) {
              ...
              int end =
                  Math.min(tail + line.length(), eventData.reply_data.length());
            ...
            Logger.i(
                "Socket unexpectedly closed while waiting for (more) response");
            changeSatisfiedState(false);
